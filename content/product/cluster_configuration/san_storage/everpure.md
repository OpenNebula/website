...
While the Everpure Storage FlashArray integration delivers full VM disk lifecycle management and the core SAN operations required by OpenNebula, it is deliberately scoped to **primary datastore provisioning** via **iSCSI block devices.**
...
{{< alert title="Warning" color="warning" >}}
The incremental backup feature of Everpure FlashArray requires the `nbd` kernel module to be loaded and the `nbdfuse` package to be installed on all OpenNebula nodes.
{{< /alert >}}
...
{{< alert title="Note" color="success" >}}
This behavior stems from the inherent complexities of iSCSI connections and is not exclusive to OpenNebula or Everpure.
{{< /alert >}}
