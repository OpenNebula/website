...
{{< image path="/images/veeam/interactive_backup_veeam_architecture.png" alt="Architecture of the OpenNebula-Veeam Backup Integration" align="center" width="90%" mb="20px" border="false" >}}
...
Edit the OneBEX configuration in the OpenNebula remotes directory and synchronize it to the Hosts:
...
To install the oVirtAPI module, configure the OpenNebula repository on the Backup Server by following [OpenNebula Repositories]({{% relref "software/installation_process/frontend_installation/opennebula_repository_configuration.md" %}}). Then install the `opennebula-ovirtapi` package.
...
As a last step, click finish. The new oVirtAPI server should be listed under Managed Servers as an **oVirt KVM** hypervisor.
