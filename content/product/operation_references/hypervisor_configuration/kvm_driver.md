...
`BALLOONING` is about dynamically adjusting the amount of RAM allocated to VMs. It allows KVM to reclaim unused memory from one VM and allocate it to another VM that needs it more, without shutting down or pausing the VMs. The parameter sets up a **balloon driver** within the VM that communicates with the Host. When the Host needs to **reclaim memory**, the driver _inflates_, reserving some of the VM’s unused memory for the Host. When the VM needs additional memory, the driver _deflates_, releasing reserved memory back to the VM.
