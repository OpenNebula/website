...
An on-prem provider is ideal for environments where you want to integrate OneForm with infrastructure you already control—such as bare-metal servers, KVM hosts, or virtual machines in your private datacenter.

## Creating an On-premises Provider
...
{{% tab header="CLI"%}}
Create an on-premises provider using the `oneprovider create <name>` command, and specifying `onprem` as the provider type. As the on-premises provider does not require access credentials, the provider is created without additional steps.
...
Once you have created the provider, review its details with the `oneprovider show <id>` command:
