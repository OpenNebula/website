...
* Image and System datastore for the cluster. The storage is configured to use the hosts’ [Local storage system datastore]({{% relref "../../../cloud_operation/cloud_clusters_infrastructure_configuration/storage_system_configuration/local_ds#local-ds" %}}). On-Premises clusters also include access to the default datastore, so you can easily share images across clusters.
