...
LXC containers need a root file system image in order to boot. This image can be downloaded directly to OpenNebula from [Linux Containers](https://uk.images.linuxcontainers.org/) Marketplace. Check the [Public Marketplaces]({{% relref "../../../apps-marketplace/marketplace_configuration/public_marketplaces/index#public-marketplaces" %}}) chapter for more information. You can use LXC with NAS (file-based), SAN (lvm) or Ceph Datastores.
