...
If you need to resize the capacity in `RUNNING` state you have to setup some extra attributes in the VM template. These attributes must be set before the VM is started. They are driver-specific, more info for [KVM]({{% relref "../../package_installation_references/kvm_node_deployment/kvm_driver#kvm-live-resize" %}}).
...
{{< alert title="Note" color="success" >}}
Check the [Monitoring Subsystem]({{% relref "../../../integration_framework/integration_references/infrastructure_drivers_development/devel-im#devel-im" %}}) guide to find out how to extend the information model and add any information probe to the Hosts.{{< /alert >}} 
