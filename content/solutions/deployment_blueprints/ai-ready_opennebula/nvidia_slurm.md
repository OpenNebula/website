...
* Configure the Slurm worker template and an example finetuning job script.
...
* [AI Factory Deployment with On-premise hardware]({{% relref "/solutions/deployment_blueprints/ai-ready_opennebula/cd_on-premises" %}})
...
2. **Adjust the SlurmWorker template**. In the Sunstone  You will add GPU and other options later in this tutorial.

3. **Instantiate the Slurm Worker(s)** via the OpenNebula Sunstone web interface:

   * Go to **Templates -> VM Templates**
   * Click on the **SlurmWorker** template.
   * Select instantiate.

![image](/images/ai_factories/instantiate-slurm-worker.png)
   ...
   * In advance options, select the **Network** tab and select **Attach NIC**:
      * Click next to **Select a network**
