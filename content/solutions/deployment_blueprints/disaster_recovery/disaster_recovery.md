---
title: "Disaster Recovery with Ceph RBD"
...
Each site contains an OpenNebula Front-end, three KVM compute nods, and an independent Ceph cluster with RDB mirroring. The Virtual Machines running production workloads reside on Site A. The Ceph clusters share the same storage network, as shown below.
...
Both sites use the same versions of software components, with the `rbd-mirror` service running on both.
...
The test environment has been configured using microenvs to deploy two separate OpenNebula clusters based on `kvm-ceph-ec#squid` and `ubuntu2204` templates with standard shapes. SSH public keys were added to all nodes on each opposite site.

## Configuration and Deployment

Setting up Ceph RBD mirroring between two sites for OpenNebula involves configuring asynchronous block-level replication to ensure VM disk images are synchronized. First, you deploy two independent Ceph clusters, one per site (Site A and Site B), with matching RBD pool names (one pool) intended for mirroring. To avoid duplicate image names, Site B does not have any images in the same pool.
