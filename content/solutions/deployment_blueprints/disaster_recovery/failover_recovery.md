...
On Site B, create a new image file for registering on Site B. This image file will be based on the information from the parent image at Site A and the Ceph virtual disk name. Note that the image file **must** be set as `persistent`.
...
Prepare a VM template file based on the desired VM from site A. You can see the VM template with `onetemplate show <TEMPLATE ID>`.

Example information for the template:
...
### If Testing Failover with Both Sites Available
